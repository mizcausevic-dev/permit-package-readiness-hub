...
<head>
  ...
  <title>${title}</title>
  <meta name="description" content="TypeScript control plane for permit packages, readiness blockers, inspection posture, and construction-safe submission operations. Operator surface in the Kinetic Gain Suite." />
  ...
  <link rel="canonical" href="https://permits.kineticgain.com/" />
  <link rel="icon" type="image/svg+xml" href="${KG_FAVICON_DATA_URI}" />
  <meta property="og:type" content="website" />
  <meta property="og:site_name" content="Kinetic Gain" />
  <meta property="og:title" content="${title}" />
  <meta property="og:url" content="https://permits.kineticgain.com/" />
  <meta property="og:description" content="TypeScript control plane for permit packages, readiness blockers, inspection posture, and construction-safe submission operations. Operator surface in the Kinetic Gain Suite." />
  <meta name="twitter:card" content="summary" />
  <meta name="twitter:title" content="${title}" />
  <meta name="twitter:description" content="TypeScript control plane for permit packages, readiness blockers, inspection posture, and construction-safe submission operations. Operator surface in the Kinetic Gain Suite." />
  <script type="application/ld+json">{"@context":"https://schema.org","@graph":[{"@type":"ProfessionalService","@id":"https://permits.kineticgain.com/#service","name":"Kinetic Gain — TypeScript control plane for permit packages, readiness blockers, inspection posture, and construction-safe submission operations","url":"https://permits.kineticgain.com/","areaServed":"United States","description":"TypeScript control plane for permit packages, readiness blockers, inspection posture, and construction-safe submission operations. Operator surface in the Kinetic Gain Suite."},{"@type":"WebSite","@id":"https://permits.kineticgain.com/#website","url":"https://permits.kineticgain.com/","name":"TypeScript control plane for permit packages, readiness blockers, inspection posture, and construction-safe submission operations","publisher":{"@type":"Organization","name":"Kinetic Gain LLC"}}]}</script>
  <style>${KG_STYLE01_CSS}${KG_COMPAT_CSS}${MENU_SURFACE_CSS}</style>
</head>
<body>
  <header>
    <div class="wrap nav">
      <a class="kg-logo" href="/" aria-label="Kinetic Gain — TypeScript control plane for permit packages, readiness blockers, inspection posture, and construction-safe submission operations">
        ${KG_MARK_SVG}
        <span class="kg-word">Kinetic Gain</span>
      </a>
      <nav class="nav-links" id="primaryNav">
        <a href="/permit-lane">Permit Lane</a>
        <a href="/readiness-risks">Readiness Risks</a>
        <a href="/inspection-posture">Inspection Posture</a>
    ...
    <div class="wrap">
      <div class="foot-top">
        <div>
          ...
          <p class="foot-tag">TypeScript control plane for permit packages, readiness blockers, inspection posture, and construction-safe submission operations — operator surface in the Kinetic Gain Suite. Static demo data; no live calls.</p>
        </div>
        <div class="foot-cols">
          <div class="foot-col">
            <h4>Estate</h4>
          ...
          <div class="foot-col">
            <h4>This surface</h4>
            <a href="https://github.com/mizcausevic-dev/permit-package-readiness-hub">GitHub repo</a>
          </div>
        </div>
      </div>
      <div class="foot-bot">
...
function topbar(active: string) {
  ...
  const links = [
    ...
    {
      "href": "/readiness-risks",
      "label": "Readiness Risks"
    },
    {
      "href": "/inspection-posture",
      "label": "Inspection Posture"
    },
    {
      "href": "/verification",
      "label": "Verification"
...
function renderHero(active: string) {
  ...
  const urgentPackage = permitLane().find((item) => item.risk === "red") ?? permitLane()[0];
  const readinessRisk = readinessRisksLane().find((item) => item.readiness === "red") ?? readinessRisksLane()[0];
  const packet = inspectionPosture().find((item) => item.status !== "green") ?? inspectionPosture()[0];
  return `${topbar(active)}
  <section class="hero reveal">
    <div class="acard hero-panel">
      <div class="hero-grid">
        <div class="hero-copy">
          <span class="eyebrow">Permit readiness</span>
          <div class="hero-meta">
          ...
          <h1>Where do permit packages quietly become jurisdiction and inspection risk?</h1>
          <p class="hero-subtle">Permit Package Readiness Hub keeps revision parity, utility sign-off, packet drift, and inspection-safe posture visible before the next jurisdiction intake or field review window burns schedule.</p>
          <div class="hero-nav">${routePills(active, links)}</div>
          <div class="hero-kpis">
            <div class="stat"><label>Packages</label><strong>${metrics.packages}</strong><span>Active permit packages tied to jurisdiction, owner, and next move.</span></div>
            <div class="stat"><label>Urgent packages</label><strong>${metrics.urgentPackages}</strong><span>Red packages where intake acceptance or schedule is already under pressure.</span></div>
            <div class="stat"><label>Blocked readiness</label><strong>${metrics.blockedReadiness}</strong><span>Evidence gaps still waiting on revision, utility, or sign-off proof.</span></div>
            <div class="stat"><label>Fragile packets</label><strong>${metrics.fragileInspectionPackets}</strong><span>Inspection packets still too weak to issue safely.</span></div>
          </div>
        </div>
        <aside class="hero-aside">
          <div class="acard">
            <span class="metric-chip">Current watch item</span>
            <h3>${urgentPackage.project} · ${urgentPackage.permitType}</h3>
            <p><strong>${urgentPackage.jurisdiction}</strong><br />${urgentPackage.excerpt}</p>
          </div>
          <div class="acard">
            <span class="metric-chip">Operator correction</span>
            <h3>What the team should do next</h3>
            <ul class="hero-mini-list">
              <li><strong>${urgentPackage.owner}</strong><span>${urgentPackage.nextAction}</span></li>
              <li><strong>${readinessRisk.owner}</strong><span>${readinessRisk.requiredEvidence}</span></li>
              <li><strong>${packet.audience}</strong><span>${packet.decisionNote}</span></li>
            </ul>
          </div>
          <div class="acard">
            <span class="metric-chip">Control surfaces</span>
            <p>Revision control, utility proof, owner sign-off, and inspection packet posture stay in one readable view instead of drifting across PMs, field teams, and jurisdiction reviewers.</p>
          </div>
        </aside>
      </div>
    </div>
  ...
  return layout(
    ...
    heroShell(
      ...
      `<section class="section">
        <div class="sh"><h2>Permit lane</h2><div class="note">Jurisdiction, revision, and inspection pressure that leadership can actually act on</div></div>
        <div class="section-cards">
          ${packages.map((item) => `<article class="acard surface-card"><span class="tag ${item.risk}">${item.risk}</span><h3>${item.project} · ${item.permitType}</h3><div class="surface-meta"><p><strong>${item.packageId}</strong> · ${item.jurisdiction}</p><p><strong>Owner:</strong> ${item.owner}</p></div><p class="surface-copy">${item.excerpt}</p><p class="surface-next"><strong>Next move:</strong> ${item.nextAction}</p></article>`).join("")}
        </div>
      </section>
      <section class="section">
        <div class="sh"><h2>Readiness pressure</h2><div class="note">Where revision, utility, and sign-off gaps can still break intake trust</div></div>
